...
#anova function not ideal because issues doing type 1 or type 2
#response variable (y) should be on left, predictor variable (x) on left
#use aov function for anova

m<-aov(length~lake,
    data=df_anova)

#summary to get more details, Pr(>F) is p-value
summary(m)
...
s2_w<-s_w/(nrow(df_anova)-n_distinct(df_anova$lake)-1)

# calculate f stat --------------------------------------------------------


f_value<-s2_b/s2_w
f_null<-seq(0,10,by=.1)
y<-df(x=f_null,
      df1=2,
      df2=147)
