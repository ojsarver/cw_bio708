...
df_co2<-rename_with(df_co2,tolower)
...
# ENTER YOUR ANSWER HERE as COMMENT: For Quebec plants, ambient CO2 was the key
#driver of assimilation rate (p-value=9.29e-08), while treatment did not
#(p-value=0.0926). This is in contrast to the Mississippi plants, as the treatment
#had a lower p-value (2.41e-08) compared to the concentration p-value (1.24e-05).
...
df_env<-rename_with(df_env,tolower)
...
df_env_sub<-df_env%>%
  select(-c(precipitation, elevation, age.cat, geology))
